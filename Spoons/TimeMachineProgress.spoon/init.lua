...
obj.backupIcon = hs.image.imageFromPath(
                     "/System/Library/CoreServices/Menu Extras/TimeMachine.menu/Contents/Resources/TMBackingUp.pdf") or
                     hs.image.imageFromAppBundle('com.apple.backup.launcher'):setSize(
                         {
            w = 18,
            h = 18
        })
...
function obj._returnMenu() return obj.menu end
...
function obj:refresh()
    ...
    local function enableCaffie()
        local isOn = hs.caffeinate.get('system')
        if not isOn then
            hs.caffeinate.set('system', true)
        else
            self.logger.d("caffie is on now, isOn: ", isOn)
        end
    end

    local function disableCaffie()
        local isOn = hs.caffeinate.get('system')
        if not isOn then
            self.logger.d("caffie is off now, isOn: ", isOn)
        else
    ...
    if data['Running'] ~= '1' then
        ...
        self.logger.df("data '%s'", hs.json.encode(data))
        ...
        local preparing = (not stopping) and
                              (data['Percent'] == '-1' or not data['Percent'] or
                                  data['Percent'] == '0')
        -- local find_changes = (not stopping) and data['BackupPhase'] == 'FindingChanges'
        ...
        if stopping then
            ...
            topmenu = tooltip
            -- elseif find_changes then
            --   title = "(find)"
            --   tooltip = "Finding Changes…"
            --   topmenu = tooltip
        elseif preparing then
            ...
            data['bytes'] = data['bytes'] or '0'
            data['totalBytes'] = data['totalBytes'] or '0'
        ...
        obj.menu = {
            ...
            -- "Skip This Backup" is disabled if the status is "Stopping" already
            {
                ...
                disabled = stopping,
                fn = function()
                    hs.task.new("/usr/bin/tmutil", nil, {
                        "stopbackup"
                    }):start()
                end
            },
