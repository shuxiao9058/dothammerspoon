...
getMailContent =  function (searchMail)
  local query
  if not searchMail or #searchMail.query == 0 then
    return
  end
  ...
  if newMailCount > 0 then
	...
	if mail and mail.thread then
	  ...
	  if not threadId or threadId < mailThreadId then
	    ...
	    logger:d("break forloop, threadId", threadId, ", maxThreadId: ", maxThreadId, ", mailThreadId: ", mailThreadId)
	    break
	  end
	end
	...
	if mail then
	  ...
	  local mailInfo = tostring(i) .. ". " .. (mail.subject or "") .. (digest or '')  .. " - " .. (mail.date_relative or "")
	  info = info .. "\n" .. mailInfo
	end
      end
    end

    if maxThreadId and maxThreadId > 0 then
...
mailDigest = function (mail)
  ...
  for _ , item in ipairs(body) do
    ...
    if content  then
      ...
      local regexpText = [[[成功率为|平均耗时] ?([?0-9\.%%]+)]]
      local slashed_text = string.match(content, regexpText)
      if slashed_text then
	return string.format(" %s ", slashed_text)
      end
    end
  end
...
local function checkUnreadMail(eventName, params)
  ...
  local output, status, _, _ = hs.execute("notmuch count " .. search .. ' ' .. " and tag:unread", true)
  local focusedStyle = {
