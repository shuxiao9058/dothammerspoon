...
local mailQueryStr = 'folder:/Work/ and tag:work and tag:unread'

local repeatTimer

local function getRecentMailList()
    local cmd = [[ notmuch search --format='json' --sort='newest-first' ]] ..
                    "'" .. mailQueryStr .. ' and thread:{date:120mins..}' .. "'"
    logger:d('cmd: ', cmd)
    local output, status, type, rc = hs.execute(cmd, true)
    if output then output = string.gsub(output, "[%s\n\r]+", "") end
    if status and output then
        logger:d('get recent unread mail info status:', status, ', output: ',
                 output, ', type: ', type, ', rc: ', rc)
    ...
    if newMailCount > 0 then
            ...
            for i = 1, newMailCount do
                ...
                if mail.thread then
                    ...
                    if not threadId or threadId < mailThreadId then
                        ...
                        logger:d('break forloop, threadId', threadId,
                                 ', maxThreadId: ', maxThreadId,
                ...
                local mailInfo = tostring(i) .. '. ' .. (mail.subject or '') ..
        ...
        local notifyAttr = {
            ...
            subTitle = string.format(
                'Received %d new mails since last refresh.', newMailCount),
...
local function checkUnreadMail(eventName, params)
    ...
    local output, status, _, _ = hs.execute("notmuch count " .. search, true)
    local focusedStyle = {
    ...
    oldUnreadMailCount = newUnreadMailCount
