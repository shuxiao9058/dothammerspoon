...
local label_length = 70 -- How wide (in characters) the dropdown menu should be. Copies larger than this will have their label truncated and end with "É" (unicode for elipsis ...)
   ...
   if (#clipboard_history == 0) then
      ...
      for k,v in pairs(clipboard_history) do
         if (string.len(v) > label_length) then
            table.insert(menuData,1, {title=string.sub(v,0,label_length).."É", fn = function() putOnPaste(v,key) end }) -- Truncate long strings
         else
